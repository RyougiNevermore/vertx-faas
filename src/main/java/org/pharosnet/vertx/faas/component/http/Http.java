...
import org.pharosnet.vertx.faas.commons.Host;
import org.pharosnet.vertx.faas.component.http.config.HttpConfig;
import org.pharosnet.vertx.faas.component.http.config.SSLConfig;
import org.pharosnet.vertx.faas.component.http.router.AbstractHttpRouter;
import org.pharosnet.vertx.faas.component.http.router.FnRouterBuilder;
...
public class Http {
    ...
    public Future<Void> run(AbstractHttpRouter routerbuilder) {
        ...
        String host = Optional.ofNullable(this.config.getHost()).orElse("").trim();
        if (host.length() == 0) {
            host = Host.get().getIp();
        }
